...
use crate::{
    array_ext::{RemoveAxisExt, ShapeExt},
    error::LinalgError,
};
    ...
    A: Scalar,
{
    ...
    fn lu(&self) -> Result<Self::Output, Self::Error> {
        ...
        let threshold = A::Real::from_f64(1e-10).ok_or_else(|| {
            LinalgError::CannotConvertElement("f64".to_string(), type_name::<A::Real>().to_string())
        })?;
...
mod tests {
    ...
    use crate::{error::LinalgError, LUDecomposable};

    #[test]
    fn lu_decomposition_works_f64() -> Result<(), LinalgError> {
